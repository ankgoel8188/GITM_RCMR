...
!           endif
        ...
        case("#RCMR")
           ...
           if(iError == 0) then
              if(RCMROutType == 'F107') then
              ...
              else if (RCMROutType == 'ManyEDC') then
                RCMRFlag = .true.
                call read_in_int(lu, iError)            !ANK: 12/04/17 Number of estimate
                allocate(RCMR_est(lu))
                allocate(RCMR_est0(lu))
                do ii = 1, lu
                  call read_in_real(RCMR_est(ii), iError)
                end do
                ...
                NorthPoleEddyCoef = RCMR_est(1)
                SouthPoleEddyCoef = RCMR_est(2)
                !Ankit 11Sept2018: Set initial u=0
                RCMR_est0 = RCMR_est
                !RCMR_est = 0
              else if (RCMROutType == 'COND') then
                 RCMRFlag = .true.
                 call read_in_int(lu, iError)            !ANK: 03/06/19             
                 allocate(RCMR_est(lu))
                 allocate(RCMR_est0(lu))
                 do ii = 1, lu
                    call read_in_real(RCMR_est(ii), iError)
                 ...
                 if(iError /= 0) then
                    write (*,*) 'Unrecognizable initial Thermal Conductivity '
                    write (*,*) 'coefficient.'
                    iError = 0
                    call stop_gitm("Check inputs (set_inputs.f90)")
                 end if
                

                 ThermalConduction_AO = RCMR_est(1)
                 !ThermalConduction_AO2 = RCMR_est(2)                              
                 !Ankit 11Sept2018: Set initial u=0   
              ...
              call read_in_int(lz, iError)            !Ankit23May16: Number of measurements   
              allocate(index_lz_tec(lz))              !Ankit14March17: index of measurement to  be assimilated                                                                                
              do ii = 1, lz
                 call read_in_int(index_lz_tec(ii), iError)
              end do
              !!call read_in_int(lu, iError)            !Ankit14March17: Number of estimates  

              call read_in_int(Nc, iError)            !Ankit23May16: Controller Order.
              call read_in_int(Nf, iError)            !Ankit14March17: Filter order            
              ALLOCATE(Nf_Array(lz*nf*lu))
              do ii = 1, (lz * nf * lu)
                call read_in_real(Nf_Array(ii), iError)
              end do
              ...
              call read_in_real(alpha, iError)        !Ankit14March17: z scaling

              call read_in_int(uFiltLength, iError)   !Ankit23May16: Length of averaging filter.
              call read_in_int(TEC_proc, iError)      !Ankit14March17: TEC_proc               
              call read_in_int(points, iError)        !Ankit14March17: Number of TEC stations 
              allocate(TEC_location(points,2))
              do ii = 1,points !while (iii < points+1)                                        
                 call read_in_real(TEC_location(ii,1), iError)
                 call read_in_real(TEC_location(ii,2), iError)
                 TEC_location(ii,1) = TEC_location(ii,1) * 3.141592653589793/180
                 TEC_location(ii,2) = TEC_location(ii,2) * 3.141592653589793/180
              end do
              if (iProc==0) then
                 write (*,*) 'RCMR inputs read!'
                 write (*,*) '>>> lz = ', lz
                 write (*,*) '>>> lu = ', lu
                 write (*,*) '>>> Nc = ', Nc
                 write (*,*) '>>> Nf = ', Nf
                 ...
                 write (*,*) '>>> P0 = ', W_Rtheta
                 write (*,*) '>>> FF = ', lambda1
                 write (*,*) '>>> RCMR will update the estimate every ', Measure_Dts*Dts, 'seconds.'
                 !write (*,*) '>>> TEC stations are on processor ', TEC_proc                  
                 !do ii = 1,points                                                            
                 !   write (*,*) '>>> TEC station #', ii, ' is at ', TEC_location(ii,1)*180/3.14, &                                                                                        
                 !        'E, ', TEC_location(ii,2)*180/3.14, 'N.'                            
                 !enddo                                                                       
              end if
           end if
           ...
           if(iError /= 0) then
             ...
             if (RCMRFlag) then
               call alloc_rcmr
               ! call init_markov_matrix  !ANK: 12/04/17 Removed Filter initialization here
               call init_rcmr
             endif
           end if

           call read_in_int(rcmrStepToTurnOff,iError)
           if(iError /= 0) then
                    write (*,*) 'Unrecognizable rcmrStepToTurnOff - '
                    write (*,*) 'Need an integer for the number of minutes for RCMR '
                    write (*,*) 'to stop updating after. i.e. 4 days * 60*24 = 5760 '
                    call stop_gitm("#RCMR incorrect by rcmrStepToTurnOff in set_inputs.f90")
            end if
            !Convert to minutes
            rcmrStepToTurnOff = rcmrStepToTurnOff*60
            write(*,*) "rcmrStepsToTurnOff", rcmrsteptoturnoff
